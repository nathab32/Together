...
inline void *av_malloc(size_t size) { return malloc(size); }
...
inline void *av_mallocz(size_t size) {
  void *ptr = av_malloc(size);
  if (ptr) memset(ptr, 0, size);
  return ptr;
}

inline int size_mult(size_t a, size_t b, size_t *r) {
  size_t t;
...
inline void *av_calloc(size_t nmemb, size_t size) {
  size_t result;
  if (size_mult(nmemb, size, &result) < 0) return NULL;
  return av_mallocz(result);
}

inline void *av_malloc_array(size_t nmemb, size_t size) {
  size_t result;
  if (size_mult(nmemb, size, &result) < 0) return NULL;
